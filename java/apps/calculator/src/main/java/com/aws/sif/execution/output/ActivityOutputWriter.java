...
@Slf4j
public class ActivityOutputWriter {
	...
	public ActivityOutputWriter(Config config, S3Utils s3, ActivitySqsWriter sqsWriter) {
		this.s3 = s3;
		this.config = config;
	...
	private void initLocalFiles(int chunkNo) throws IOException {
		...
		this.activityValuePath = Files.createTempFile((String.valueOf(chunkNo)+"-"+config.getString("calculator.upload.s3.activityValues.name")+"_"), ".csv");
