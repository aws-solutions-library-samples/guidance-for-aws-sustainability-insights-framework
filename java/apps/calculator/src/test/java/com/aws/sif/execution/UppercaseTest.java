...
public class UppercaseTest extends CalculatorBaseTest {
    ...
    private static Stream<Arguments> providerForSuccess() {
        return Stream.of(
                Arguments.of("uppercase('abc')", EvaluateResponse.builder().result(new StringTypeValue("ABC"))
                        .evaluated(Map.of("uppercase('abc')", "ABC")).build()),
			Arguments.of("uppercase(concat('a','b','c'))", EvaluateResponse.builder().result(new StringTypeValue("ABC"))
				.evaluated(Map.of(
					"concat('a','b','c')", "abc",
					"uppercase(concat('a','b','c'))", "ABC")
				).build())
        );
    }
