...
public class S3Utils {
    ...
    public void uploadDirectory(@NotNull S3Location req, Path path) {
        log.debug("upload> in> req:{}", req);
        log.trace("upload> in> Path:{}", path.toString());
        ...
        try {
            ...
            for (File file : files) {
                var putObj = PutObjectRequest.builder()
                        ...
                        .key(req.getKey() + file.getName())
                        .build();

                var future = s3.putObject(putObj, AsyncRequestBody.fromFile(file));
                future.join();
            }
    ...
    public void uploadFile(S3Location req, String path) {
        log.debug("upload> in> req:{}", req);
        ...
        try {
            ...
            var future = s3.putObject(putObj, AsyncRequestBody.fromFile(new File(path)));
            future.join();
