...
import { signedUrlResponse, SignedUrlResponse } from '../schemas.js';
import { referenceDatasetFileContentExample, signedUrlResponseExample } from '../examples.js';

export default function createReferenceDatasetDataDownloadRoute(fastify: FastifyTypebox, _options: unknown, done: () => void): void {
	fastify.route({
		method: 'GET',
		url: '/referenceDatasets/:id/data',
		schema: {
...
`,
		...
		handler: async (request, reply) => {
			...
			if (request.headers['accept'] === 'application/json') {
				const referenceDatasetContent: SignedUrlResponse = await svc.getReferenceDatasetDownloadUrl(request.authz, request.params.id, undefined);
				await reply.status(200).type('application/json').send(referenceDatasetContent);
			} else {
				const referenceDatasetContent = await svc.getContent(request.authz, request.params.id);
				await reply
					.status(200)
