...
export default function createReferenceDatasetIndexDownloadRoute(fastify: FastifyTypebox, _options: unknown, done: () => void): void {
	fastify.route({
		method: 'GET',
		url: '/referenceDatasets/:id/index',
		schema: {
...
`,
		...
		handler: async (request, reply) => {
			...
			const referenceDatasetContent: SignedUrlResponse = await svc.getReferenceDatasetIndexDownloadUrl(request.authz, request.params.id, undefined);
