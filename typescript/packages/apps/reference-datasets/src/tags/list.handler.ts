...
import { commonHeaders, countPaginationQS, forbiddenResponse, apiVersion100, FastifyTypebox, fromValuePaginationQS, keyParam, parentValueQS, TagValuesListResource, tagValuesListResource, TagService } from '@sif/resource-api-base';
...
`,
		...
		handler: async (request, reply) => {
			...
			const { count, fromValue, parentValue } = request.query;
			...
			const [values, lastEvaluatedValue] = await svc.listByGroupId(request.authz, key, {
				...
				resourceKeyPrefix: PkType.ReferenceDataset,
			});
