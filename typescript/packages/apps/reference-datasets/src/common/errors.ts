...
export function handleError(error, _request, reply) {
	...
	if (error.statusCode === 400 || Array.isArray(error.validation)) {
		...
		switch (error.name) {
			...
			case 'DuplicateTagsError':
			case 'QueryParameterError':
			case 'InvalidRequestError':
			case 'ReferenceDatasetDefinitionError':
				return reply.badRequest(error.message);
...
export class InvalidFileHeaderError extends Error {
	public constructor(message: string) {
		super(message);
		this.name = 'InvalidFileHeaderError';
	}
}
...
export class ReferenceDatasetDefinitionError extends Error {
	public constructor(message: string) {
		super(message);
		this.name = 'ReferenceDatasetDefinitionError';
	}
}
