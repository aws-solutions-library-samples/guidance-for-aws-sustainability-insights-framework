...
export function handleError(error, _request, reply) {
	...
	if (error.statusCode === 400 || Array.isArray(error.validation)) {
		...
		switch (error.name) {
			...
			case 'ActivityDefinitionError':
			case 'QueryParameterError':
			case 'InvalidRequestError':
				return reply.badRequest(error.message);
			case 'ActivityTaskDefinitionError':
				return reply.badRequest(error.message);
			case 'NotFoundError':
...
export class ActivityDefinitionError extends Error {
	public constructor(message: string) {
		super(message);
		this.name = 'ActivityDefinitionError';
	}
}

export class ActivityTaskDefinitionError extends Error {
	public constructor(message: string) {
		super(message);
		this.name = 'ActivityTaskDefinitionError';
	}
}
