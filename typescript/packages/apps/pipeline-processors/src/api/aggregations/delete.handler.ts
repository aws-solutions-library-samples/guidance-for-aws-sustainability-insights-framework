...
export default function deleteMetricAggregationJobRoute(fastify: FastifyTypebox, _options: unknown, done: () => void): void {
	fastify.route({
		method: 'DELETE',
		url: '/aggregations/:id',
		schema: {
			description: `Delete Metric Aggregation Job

Permissions:
- Only \`Admin\` may delete metric aggregation Job.
`,
