...
import type { DataResultProcessorTaskHandler, ImpactCreationTaskEvent } from '../tasks/model.js';
import type { DataResultProcessorTask } from '../tasks/dataResultProcessorTask.js';
import { validateNotEmpty } from '@sif/validators';

const app: FastifyInstance = await buildLightApp();
const di: AwilixContainer = app.diContainer;

const task = di.resolve<DataResultProcessorTask>('dataResultProcessorTask');

export const handler: DataResultProcessorTaskHandler = async (event, _context, _callback): Promise<ImpactCreationTaskEvent> => {
	...
	return { executionId, pipelineId, security, pipelineType, errorLocationList };
};
