...
describe('ImpactCreationTask', () => {
	...
	let mockGetSecurityContext: GetSecurityContext = (() => {
	}) as any;
	...
	beforeEach(() => {
		...
		underTest = new ImpactCreationTask(logger, mockedS3Client as unknown as S3Client, 'bucket', mockImpactClient, mockGetLambdaRequestContext, mockGetSecurityContext);
	...
	('update activity [happy path', async (data, expectedNewActivity) => {
		...
		await underTest.process([{
			pipelineId: 'pipe1', executionId: 'exec1'
