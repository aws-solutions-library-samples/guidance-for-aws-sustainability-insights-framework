...
import type { MetricAggregationTaskEvent } from '../tasks/model.js';
import { validateDefined, validateHasSome, validateNotEmpty } from '@sif/validators';

import type { LambdaRequestContext, Metric as MetricResource, MetricClient } from '@sif/clients';
...
import type { AggregationTaskAuroraRepository } from '../services/aggregationTask.aurora.repository.js';
import type { MetricAggregationRepository } from '../services/metricAggregationRepository.js';
...
export class MetricAggregationTaskV2 {
	...
	public async process(event: MetricAggregationTaskEvent): Promise<MetricAggregationTaskEvent> {
		this.log.info(`MetricAggregationTaskV2> process> event: ${JSON.stringify(event)}`);
		...
		this.log.trace(`MetricAggregationTaskV2> process> aggregating metric ${metricToProcess.metric}`);
		...
		this.log.info(`MetricAggregationTaskV2> process> exit:`);
	...
	private async rollupMetric(groupHierarchy: string[], timeRange: AffectedTimeRange, metric: MetricResource, triggeringPipelineId: string, triggeringExecutionId: string): Promise<void> {
		this.log.info(`MetricAggregationTaskV2> rollupMetric> processing groupHierarchy:${JSON.stringify(groupHierarchy)}, timeRange:${JSON.stringify(timeRange)}, metric name:${metric.name}, id:${metric.id}`);
		...
		for (let groupIndex = 0; groupIndex < groupHierarchy.length; groupIndex++) {
			...
			this.log.debug(`MetricAggregationTaskV2> rollupMetric> processing groupId:${groupId}`);
			...
			for (const { fromUnit, toUnit } of rollupTo) {
				this.log.debug(`MetricAggregationTaskV2> rollupMetric> fromUnit:${fromUnit}, toUnit:${toUnit}, aggregated:${JSON.stringify(timeRange)}`);
				...
				this.log.debug(`MetricAggregationTaskV2> rollupMetric> timeRangeTimeUnit:${JSON.stringify(timeRangeTimeUnit)}`);
		...
		this.log.debug(`MetricAggregationTaskV2> rollupMetric> exit:`);
	}
