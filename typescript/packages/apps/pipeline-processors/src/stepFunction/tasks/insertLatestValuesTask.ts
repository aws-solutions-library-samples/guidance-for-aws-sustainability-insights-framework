...
export class InsertLatestValuesTask {
	...
	public async process(event: ProcessedTaskEvent): Promise<Status> {
		this.log.info(`InsertLatestValuesTask> process> event: ${JSON.stringify(event)}`);
		...
		try {
			...
			status = 'FAILED';
			this.log.error(`InsertLatestValuesTask> process> error: ${JSON.stringify(Exception)}`);
		} finally {
			if (sharedDbConnection !== undefined) {
				await sharedDbConnection.end();
			}
		}
		this.log.info(`InsertLatestValuesTask> exit:`);
		return status;
	}
}
