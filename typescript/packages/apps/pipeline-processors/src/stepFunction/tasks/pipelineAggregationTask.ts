...
export class PipelineAggregationTask {
	...
	public async process(event: ProcessedTaskEvent): Promise<void> {
		this.log.info(`PipelineAggregationTask> process> event: ${JSON.stringify(event)}`);
		...
		if (requiresAggregation) {
			this.log.info(`PipelineAggregationTask> process> pipeline ${pipelineId} has aggregation specified`);
			...
			this.log.debug(`PipelineAggregationTask> process> executionGroups: ${JSON.stringify(executionGroups)}`);
			...
			for await (const group of executionGroups) {
				for await (const [activities, fromOffset] of this.getActivitiesAggregatedBySqlFunctions(pipelineId, executionId, group, metadata)) {
					this.log.info(`PipelineAggregationTask> process> fromOffset: ${fromOffset}`);
		...
		this.log.info(`PipelineAggregationTask> process> exit:`);
	}
	...
	public async* getActivitiesAggregatedBySqlFunctions(pipelineId: string, executionId: string, groupContextId: string, pipelineMetadata: PipelineMetadata) {
		this.log.debug(`PipelineAggregationTask> getActivitiesAggregatedBySqlFunctions> in> pipeline ${pipelineId}, executionId: ${executionId}, groupContextId: ${groupContextId}, pipelineMetadata: ${pipelineMetadata}`);
		...
		this.log.debug(`PipelineAggregationTask> getActivitiesAggregatedBySqlFunctions> out>`);
	}
}
