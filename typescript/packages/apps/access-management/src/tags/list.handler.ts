...
`,
			...
			querystring: Type.Object({
				...
				resourceType: stringEnum(['user', 'group'], 'Resource type'),
			}),
			response: {
				200: {
					description: 'Success.',
		...
		handler: async (request, reply) => {
			...
			switch (resourceType) {
				case 'user':
					resourceKeyPrefix = PkType.User;
					break;
				case 'group':
					resourceKeyPrefix = PkType.Group;
					break;
				default:
					return reply.badRequest('Unrecognized ?resourceType provided.');
			}
