...
Before({ tags: '@setup_tenantToTenant' }, async function () {
	const nodeEnv = process.env.NODE_ENV as string;
	global.apicklis = {
		accessManagement: await createApi(nodeEnv, process.env.ACCESS_MANAGEMENT_BASE_URL as string, {}),
		sharedTenantAccessManagement: await createApi(nodeEnv, process.env.SHARED_TENANT_ACCESS_MANAGEMENT_BASE_URL as string, {}),
		...
		sharedCalculations: await createApi(nodeEnv, process.env.SHARED_TENANT_CALCULATIONS_BASE_URL as string, {}),
		sharedImpacts: await createApi(nodeEnv, process.env.SHARED_TENANT_IMPACTS_BASE_URL as string, {}),
		sharedReferenceDatasets: await createApi(nodeEnv, process.env.SHARED_TENANT_REFERENCE_DATASETS_BASE_URL as string, {}),
