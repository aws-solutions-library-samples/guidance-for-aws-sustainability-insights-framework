...
Feature:
	...
	Scenario: Admin can create new metric
		...
		And I set body to {"name":"ghg:scope1:mobile:site1","summary":"GHG Scope 1 direct emissions from mobile combustion in site1.","aggregationType":"sum","outputMetrics":["ghg:scope1:mobile"],"tags":{"standard":"ghg","scope":"1","category":"mobile", "site": "site1"}}
		When I POST to /metrics
		Then response code should be 201
		And response body should contain id
		And I store the value of body path $.id as metric_scope1_mobile_site1_id in global scope
		And response body path $.name should be ghg:scope1:mobile:site1
		...
		And response body path $.outputMetrics[0] should be ghg:scope1:mobile
	...
	Scenario: Admin cannot modify metric output that resulted in circular dependency
		Given I authenticate using email metricsApiTests_admin@amazon.com and password p@ssword1
		And I set body to {"name":"ghg:scope1","summary":"GHG Scope 1 direct emissions.","aggregationType":"sum","tags":{"standard":"ghg","scope":"1"}, "outputMetrics":["ghg:scope1:mobile:site1"]}
		When I PATCH /metrics/`metric_scope1_id`
		Then response code should be 400
		And response body path $.message should be There is circular reference pointing back to metric ghg:scope1 from ghg:scope1:mobile
	...
	For example, if we have ghg:scope1:mobile:site1 -> ghg:scope1:mobile -> ghg:scope1
	then we should not be able to create metric that output to ghg:scope1:mobile:site1 and ghg:scope1:mobile
		Given I authenticate using email metricsApiTests_admin@amazon.com and password p@ssword1
		And I set body to {"name":"ghg:invalid:multiple:metrics","summary":"GHG Scope 1 direct emissions.","aggregationType":"sum","tags":{"standard":"ghg","scope":"1"}, "outputMetrics":["ghg:scope1:mobile:site1", "ghg:scope1:mobile"]}
		When I POST to /metrics
		Then response code should be 400
		And response body path $.message should be This metric ghg\:scope1\:mobile exists in multiple path
	...
	Scenario: Admin cannot delete metric if it is configured as an output of another metric
		...
		Then response code should be 400
		And response body path $.message should be metric ghg:scope1:mobile cannot be deleted because it is an output metric of another metric or pipeline
