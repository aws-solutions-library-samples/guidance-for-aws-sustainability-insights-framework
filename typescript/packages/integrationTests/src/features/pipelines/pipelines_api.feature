...
Feature:
	...
	Scenario: Create Pipeline with type impacts should fails when required column(s) are not specified
		...
		When I POST to /pipelines
		Then response code should be 400
		And response body path $.message should be Missing mandatory output columns. Missing column 'activity:name' used to specify the activity name.

	Scenario: Admin can create new impacts type pipeline
		Given I authenticate using email pipelinesApiTests_admin@amazon.com and password p@ssword1
		And I set body to { "type": "impacts", "connectorConfig": { "input": [ { "name": "sif-csv-pipeline-input-connector" } ] }, "attributes": { "type": "E2E" }, "name": "test2", "description": "E2E test pipeline", "transformer": { "transforms": [ { "index": 0, "formula": "CONCAT('eiolca',:product)", "outputs": [ { "index": 0, "key": "activity:name", "type": "string" } ] }, { "index": 1, "formula": "'ghg_emissions'", "outputs": [ { "index": 0, "key": "impact:impactKey:name", "type": "string" } ] }, { "index": 2, "formula": "'co2e'", "outputs": [ { "index": 0, "key": "impact:impactKey:component:co2e:key", "type": "string" } ] }, { "index": 3, "formula": "SWITCH(:chosen_value,1,:co2ePerDollar_1,2,co2ePerDollar_2,3,co2ePerDollar_3,4,co2ePerDollar_4,5,co2ePerDollar_5)", "outputs": [ { "index": 0, "key": "impact:impactKey:component:co2e:value", "type": "number" } ] }, { "index": 4, "formula": "'pollutant'", "outputs": [ { "index": 0, "key": "impact:impactKey:component:co2e:type", "type": "string" } ] } ], "parameters": [ { "index": 0, "key": "product", "type": "string" }, { "index": 0, "key": "chosen_value", "type": "number" }, { "index": 0, "key": "co2ePerDollar_1", "type": "number" }, { "index": 0, "key": "co2ePerDollar_2", "type": "number" }, { "index": 0, "key": "co2ePerDollar_3", "type": "number" }, { "index": 0, "key": "co2ePerDollar_4", "type": "number" }, { "index": 0, "key": "co2ePerDollar_5", "type": "number" } ] } }
		When I POST to /pipelines
		Then response code should be 201
