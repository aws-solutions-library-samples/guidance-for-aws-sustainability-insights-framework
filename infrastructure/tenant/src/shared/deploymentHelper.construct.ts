	...
	constructor(scope: Construct, id: string, props?: DeploymentHelperConstructProperties) {
				...
				statements: [
					new PolicyStatement({
						...
						resources: [
							...
							`arn:aws:glue:${region}:${accountId}:table/${props.auditLogsDatabaseName}/${props.auditLogsTableName}-v1`
						]
					})]
			})
		);
		...
		NagSuppressions.addResourceSuppressions([deploymentHelperLambda], [
			...
			},
			{
				...
				reason: 'This policy allows the deployment helper to add indexes on new glue table versions'
			},
		], true);
